...
**/
/**
 * OLED_GRAM[128][8]中的128代表列数（x坐标），
 * 而8代表的是页，每页又包含8行，
 * 总共是64行（y坐标）。从高到低对应行数从小到大
 * 比如，我们要在x=100，y=29这个点写入1，则可以用这个句子实现：
 * OLED_GRAM[100][4]=1<<2;
**/
 ...
 * @param   x: the X-axis of cursor (max 128)
 * @param   y: the Y-axis of cursor (max 64)
...
/**
 * @brief   deal with the oled button
 * @param   None
 * @retval  the button value(中左右上下 对应12345)
 */
uint8_t get_button_value(float voltage)
{
	if(voltage < (0.68f -0.1f))
		return 1;
	else if(voltage < (1.39f-0.1f))
		return 2;
	else if(voltage < (1.98f-0.1f))
		return 3;
	else if(voltage < (2.64f-0.1f))
		return 4;	
	else
		return 5;
}
